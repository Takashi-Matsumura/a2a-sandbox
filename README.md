...
1. **デモ一覧ページ** (`/demo`) から始めて、2つのデモを体験
2. **会議スケジューリングデモ** でプライバシー保護付きの協調動作を観察
3. **ディベートデモ** でLLM連携によるエージェント間の対話フローを体験
4. **A2A Protocol メッセージ** パネルで JSON-RPC のリクエスト/レスポンスを確認
5. **エージェントページ** で個別のエージェントとチャット
6. **Agent Card タブ** でメタデータ仕様を理解
